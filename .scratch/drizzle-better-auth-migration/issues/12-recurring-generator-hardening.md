# 12 — Automated Recurring Transactions Generator Hardening & Refactoring
...
1. **Explicit Circuit Breaker Resets**: Modify `updateRecurringTransaction()` so `failureCount` and `lastFailureReason` are reset ONLY when `active: true` is explicitly passed (e.g., when the user clicks "Resume" or explicitly toggling active state).
...
- Changing database schema columns (schema already supports `schedule_mode`, `failure_count`, and `last_failure_reason`).
- Altering the cron endpoint authentication mechanism (`CRON_SECRET` bearer token).

## Further Notes

- Relies on existing Drizzle schema and Base UI drawer primitives.
