...
if echo "$command_line" | grep -qE '\b(npm|yarn)\b'; then
  cat <<JSON_OUT
{
  "allow_tool": false,
  "deny_reason": "Do not use npm or yarn. Please use pnpm for package management and scripts (e.g., pnpm install, pnpm dev, pnpm build)."
}
JSON_OUT
  exit 0
fi
