...
describe("OnboardingWizard Rendering", () => {
  it("renders step 1 with workspace mode choices and progress indicator", () => {
    const html = renderToString(<OnboardingWizard />)

    expect(html).toContain("Select Workspace Mode")
    expect(html).toContain("Personal Mode")
    expect(html).toContain("Business Mode")
...
describe("OnboardingWizard Step Navigation & Preference Saving", () => {
  ...
  it("saves preferences to useAppStore and triggers toast.success on completing wizard", async () => {
    ...
    await act(async () => {
      root.render(<OnboardingWizard onComplete={onCompleteMock} />)
    })

    // Step 1: Select Business Mode
